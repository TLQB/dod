...
class DataProcessingForm(forms.Form):
    ...
    output_filename = forms.CharField(
        label="Output Filename", required=True, initial="output_excel.xlsx"
    )
    sheet_name = forms.MultipleChoiceField(
...
class DataUpdateDiscountForm(forms.Form):
    input_filename = forms.FileField(
        label="Data File",
        validators=[FileExtensionValidator(allowed_extensions=["xlsx"])],
    )
    output_filename = forms.FileField(
        label="Update File",
        validators=[FileExtensionValidator(allowed_extensions=["xlsx"])],
    )
